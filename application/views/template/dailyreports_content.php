<?php 
$fromdate=$_POST['fromdate']; //post value of the date
$tilldate=$_POST['tilldate']; //post value of the date
...
<?php
			...
			$query = $this->db->query('SELECT receipt_no,pos_invoice.*,customers.customer_name FROM pos_payment 
							...
							LEFT JOIN customers
							ON pos_invoice.customer_id=customers.customer_id WHERE pos_invoice.transaction_date BETWEEN CAST('.$filterdate.' AS DATE) AND CAST('.$filterdate2.' AS DATE)');
$gtotal = $query->row();
