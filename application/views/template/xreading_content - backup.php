				...
				<td width="45%" valign="top">
                    <companyname>
                        ...
                        <strong><?php  echo "Date :<u>".$today."</u> Time : <u>".$time."</u>";?></strong><br>
                          <strong><?php echo "Cashier Name :".$this->session->user_fullname; ?> </strong>
                    </companyname>
                </td>
...
<?php
			...
			$query = $this->db->query('SELECT receipt_no,pos_invoice.*,customers.customer_name,pos_payment_details.cash_amount FROM pos_payment 
							LEFT JOIN pos_invoice
							ON pos_payment.pos_invoice_id=pos_invoice.pos_invoice_id 
							LEFT JOIN pos_payment_details
							ON pos_payment.pos_payment_id=pos_payment_details.pos_payment_id 
							LEFT JOIN customers
							ON pos_invoice.customer_id=customers.customer_id WHERE user_id='.$user_id.' AND end_batch="1" ');
			$gtotal = $query->row();
...
echo "Aw";
			...
			<?php echo "<p style='text-align:center;font-family:tahoma;font-size:11px'>".$notes->note1 . "<br>" . $notes->note2 ."<br>" .
			$notes->note3 ."<br>" . $notes->note4 ."<br>" . $notes->note5 ."<br>" . $notes->note6 ."<br>" . $notes->note7 ."<br>" . $notes->note8 .
			"<br>" . $notes->note9 ."<br>" . $notes->note10 ."</p>"; ?>
